...
function ContextMenuItem({ onSelect, children }: { onSelect?: () => void; children: ReactNode }) {
  return (
    <ContextMenu.Item
      onSelect={onSelect}
      className="flex items-center gap-2 px-2 py-1.5 outline-0 text-sm text-fmc-elements-textPrimary cursor-pointer ws-nowrap text-fmc-elements-item-contentDefault hover:text-fmc-elements-item-contentActive hover:bg-fmc-elements-item-backgroundActive rounded-md"
    >
...
function InlineInput({ depth, placeholder, initialValue = '', onSubmit, onCancel }: InlineInputProps) {
  ...
  return (
    <div
      className="flex items-center w-full px-2 bg-fmc-elements-background-depth-4 border border-fmc-elements-item-contentAccent py-0.5 text-fmc-elements-textPrimary"
      style={{ paddingLeft: `${6 + depth * NODE_PADDING_LEFT}px` }}
    >
      <div className="scale-120 shrink-0 i-ph:file-plus text-fmc-elements-textTertiary" />
      <input
        ref={inputRef}
        type="text"
        className="ml-2 flex-1 bg-transparent border-none outline-none py-0.5 text-sm text-fmc-elements-textPrimary placeholder:text-fmc-elements-textTertiary min-w-0"
...
function FileContextMenu({
  ...
  const handleDelete = async () => {
    try {
      if (!confirm(`Voulez-vous vraiment supprimer le ${isFolder ? 'dossier' : 'fichier'} : ${fileName} ?`)) {
        return;
      }
      ...
      if (success) {
        ...
        toast.error(`Échec de la suppression du ${isFolder ? 'dossier' : 'fichier'}`);
      }
    } catch (error) {
      toast.error(`Erreur lors de la suppression du ${isFolder ? 'dossier' : 'fichier'}`);
      logger.error(error);
    }
  };
  ...
  const handleLockFile = () => {
    try {
      ...
      if (success) {
        toast.success(`Fichier verrouillé avec succès`);
      } else {
        toast.error(`Échec du verrouillage du fichier`);
      }
    } catch (error) {
      toast.error(`Erreur lors du verrouillage du fichier`);
      logger.error(error);
    }
  };
  ...
  const handleUnlockFile = () => {
    try {
      ...
      toast.error(`Erreur lors du déverrouillage du fichier`);
      logger.error(error);
    }
  };
  ...
  const handleLockFolder = () => {
    try {
      ...
      if (success) {
        toast.success(`Dossier verrouillé avec succès`);
      } else {
        toast.error(`Échec du verrouillage du dossier`);
      }
    } catch (error) {
      toast.error(`Erreur lors du verrouillage du dossier`);
      logger.error(error);
    }
  };
  ...
  const handleUnlockFolder = () => {
    try {
      ...
      if (success) {
        toast.success(`Dossier déverrouillé avec succès`);
      } else {
        toast.error(`Échec du déverrouillage du dossier`);
      }
    } catch (error) {
      toast.error(`Erreur lors du déverrouillage du dossier`);
      logger.error(error);
    }
  };
    ...
    <>
      <ContextMenu.Root>
        <ContextMenu.Trigger>
          <div
            ...
            className={classNames('relative', {
              'bg-fmc-elements-background-depth-2 border border-dashed border-fmc-elements-item-contentAccent rounded-md':
                isDragging,
            })}
          >
            {children}
        ...
        <ContextMenu.Portal>
          <ContextMenu.Content
            style={{ zIndex: 998 }}
            className="border border-fmc-elements-borderColor rounded-md z-context-menu bg-fmc-elements-background-depth-1 dark:bg-fmc-elements-background-depth-2 data-[state=open]:animate-in animate-duration-100 data-[state=open]:fade-in-0 data-[state=open]:zoom-in-98 w-56"
          >
            <ContextMenu.Group className="p-1 border-b-px border-solid border-fmc-elements-borderColor">
            ...
            <ContextMenu.Group className="p-1 border-t-px border-solid border-fmc-elements-borderColor">
              {!isFolder ? (
                <>
            ...
            <ContextMenu.Group className="p-1 border-t-px border-solid border-fmc-elements-borderColor">
              <ContextMenuItem onSelect={handleDelete}>
                <div className="flex items-center gap-2 text-red-500">
                  ...
                  Supprimer le {isFolder ? 'dossier' : 'fichier'}
                </div>
              </ContextMenuItem>
            </ContextMenu.Group>
          </ContextMenu.Content>
    ...
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={folder.fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent text-fmc-elements-item-contentDefault hover:text-fmc-elements-item-contentActive hover:bg-fmc-elements-item-backgroundActive':
            !selected,
          'bg-fmc-elements-item-backgroundAccent text-fmc-elements-item-contentAccent': selected,
        })}
        depth={folder.depth}
        ...
        <div className="flex items-center w-full">
          ...
          {isLocked && (
            <span
              ...
              title={'Le dossier est verrouillé'}
            />
          )}
        </div>
      </NodeButton>
...
function File({
  ...
  return (
    <FileContextMenu onCopyPath={onCopyPath} onCopyRelativePath={onCopyRelativePath} fullPath={fullPath}>
      <NodeButton
        className={classNames('group', {
          'bg-transparent hover:bg-fmc-elements-item-backgroundActive text-fmc-elements-item-contentDefault':
            !selected,
          'bg-fmc-elements-item-backgroundAccent text-fmc-elements-item-contentAccent': selected,
        })}
        depth={depth}
        iconClasses={classNames('i-ph:file-duotone scale-98', {
          'group-hover:text-fmc-elements-item-contentActive': !selected,
        })}
        onClick={onClick}
      >
        <div
        ...
        >
          ...
          <div className="flex items-center gap-1">
            ...
            {locked && (
              <span
                className={classNames('shrink-0', 'i-ph:lock-simple scale-80 text-red-500')}
                title={'Le fichier est verrouillé'}
              />
            )}
