...
export const ToolInvocations = memo(({ toolInvocations, toolCallAnnotations, addToolResult }: ToolInvocationsProps) => {
  ...
  return (
    <div className="tool-invocation border border-fmc-elements-borderColor flex flex-col overflow-hidden rounded-lg w-full transition-border duration-150">
      <div className="flex">
        <button
          className="flex items-stretch bg-fmc-elements-background-depth-2 hover:bg-fmc-elements-artifacts-backgroundHover w-full overflow-hidden"
          onClick={toggleDetails}
          aria-label={showDetails ? 'Réduire les détails' : 'Afficher les détails'}
        >
          <div className="p-2.5">
            <div className="i-ph:wrench text-xl text-fmc-elements-textSecondary hover:text-fmc-elements-textPrimary transition-colors"></div>
          </div>
          <div className="p-2.5 w-full text-left">
            <div className="w-full text-fmc-elements-textPrimary font-medium leading-5 text-sm">
              Invocations d'outils MCP{' '}
              {hasToolResults && (
                <span className="w-full w-full text-fmc-elements-textSecondary text-xs mt-0.5">
                  ({toolResults.length} {toolResults.length > 1 ? 'outils utilisés' : 'outil utilisé'})
                </span>
              )}
            </div>
          </div>
        ...
        <AnimatePresence>
          {hasToolResults && (
            <motion.button
              ...
              className="bg-fmc-elements-artifacts-background hover:bg-fmc-elements-artifacts-backgroundHover"
              onClick={toggleDetails}
            >
              <div className="p-2">
                <div
                  className={`${showDetails ? 'i-ph:caret-up-bold' : 'i-ph:caret-down-bold'} text-xl text-fmc-elements-textSecondary hover:text-fmc-elements-textPrimary transition-colors`}
                ></div>
              </div>
            </motion.button>
          )}
      ...
      <AnimatePresence>
        {hasToolCalls && (
          ...
          >
            <div className="bg-fmc-elements-artifacts-borderColor h-[1px]" />

            <div className="px-3 py-3 text-left bg-fmc-elements-background-depth-2">
              <ToolCallsList
        ...
        {hasToolResults && showDetails && (
          ...
          >
            ...
            <div className="p-5 text-left bg-fmc-elements-actions-background">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5 text-xs mb-1">
                {isErrorResult ? (
                  <div className="text-lg text-fmc-elements-icon-error">
                    <div className="i-ph:x"></div>
                  </div>
                ) : (
                  <div className="text-lg text-fmc-elements-icon-success">
                    <div className="i-ph:check"></div>
                  </div>
                )}
                <div className="text-fmc-elements-textSecondary text-xs">Serveur :</div>
                <div className="text-fmc-elements-textPrimary font-semibold">{annotation?.serverName}</div>
              </div>

              <div className="ml-6 mb-2">
                <div className="text-fmc-elements-textSecondary text-xs mb-1">
                  Outil : <span className="text-fmc-elements-textPrimary font-semibold">{toolName}</span>
                </div>
                <div className="text-fmc-elements-textSecondary text-xs mb-1">
                  Description :{' '}
                  <span className="text-fmc-elements-textPrimary font-semibold">{annotation?.toolDescription}</span>
                </div>
                <div className="text-fmc-elements-textSecondary text-xs mb-1">Paramètres :</div>
                ...
                <div className="text-fmc-elements-textSecondary text-xs mt-3 mb-1">Résultat :</div>
                <div className="bg-[#FAFAFA] dark:bg-[#0A0A0A] p-3 rounded-md">
      ...
      <ul className="list-none space-y-4">
        {toolInvocations.map((tool, index) => {
          ...
          return (
            ...
            >
              <div className="bg-fmc-elements-background-depth-3 rounded-lg p-2">
                <div key={toolCallId} className="flex gap-1">
                  <div className="flex flex-col items-center ">
                    <span className="mr-auto font-light font-normal text-md text-fmc-elements-textPrimary rounded-md">
                      {toolName}
                    </span>
                    <span className="text-xs text-fmc-elements-textSecondary font-light break-words max-w-64">
                  ...
                  <div className="flex items-center justify-end gap-2 ml-auto">
                    <button
                      className={classNames(
                        ...
                        'bg-transparent',
                        'text-fmc-elements-textTertiary hover:text-fmc-elements-textPrimary',
                    ...
                    >
                      Annuler <span className="opacity-70 text-xs ml-1">{isMac ? '⌘⌫' : 'Ctrl+Backspace'}</span>
                    </button>
                    <button
                      className={classNames(
                        ...
                        'text-accent-500 hover:text-fmc-elements-textPrimary',
                    ...
                    >
                      Exécuter l'outil <span className="opacity-70 text-xs ml-1">{isMac ? '⌘↵' : 'Ctrl+Enter'}</span>
                    </button>
                  </div>
                </div>
              </div>
