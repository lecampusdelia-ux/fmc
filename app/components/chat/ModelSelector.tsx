...
export const ModelSelector = ({
  ...
  if (providerList.length === 0) {
    return (
      <div className="mb-2 p-4 rounded-lg border border-fmc-elements-borderColor bg-fmc-elements-prompt-background text-fmc-elements-textPrimary">
        <p className="text-center">
          Aucun fournisseur n'est actuellement activé. Veuillez activer au moins un fournisseur dans les paramètres pour commencer à discuter.
        </p>
      </div>
    );
  }
      ...
      <div className="relative flex w-full" onKeyDown={handleProviderKeyDown} ref={providerDropdownRef}>
        <div
          className={classNames(
            'w-full p-2 rounded-lg border border-fmc-elements-borderColor',
            'bg-fmc-elements-prompt-background text-fmc-elements-textPrimary',
            'focus-within:outline-none focus-within:ring-2 focus-within:ring-fmc-elements-focus',
            'transition-all cursor-pointer',
            isProviderDropdownOpen ? 'ring-2 ring-fmc-elements-focus' : undefined,
          )}
                  ...
                  className={classNames(
                    ...
                    localProviderStatus[provider.name] === 'connected'
                      ...
                      : localProviderStatus[provider.name] === 'disconnected'
                        ? 'bg-red-400'
                        : 'bg-fmc-elements-textTertiary',
                  )}
                  title={
                    localProviderStatus[provider.name] === 'connected'
                      ? `${provider.name} est en cours d'exécution`
                      : localProviderStatus[provider.name] === 'disconnected'
                        ? `${provider.name} n'est pas joignable`
                        : 'Vérification...'
                  }
                />
              )}
              {provider?.name || 'Sélectionnez un fournisseur'}
            </div>
            <div
              className={classNames(
                'i-ph:caret-down w-4 h-4 text-fmc-elements-textSecondary opacity-75',
        ...
        {isProviderDropdownOpen && (
          <div
            className="absolute z-20 w-full mt-1 py-1 rounded-lg border border-fmc-elements-borderColor bg-fmc-elements-background-depth-2 shadow-lg"
            role="listbox"
            id="provider-listbox"
          >
            <div className="px-2 pb-2">
              <div className="relative">
                <input
                  ...
                  placeholder="Rechercher des fournisseurs... (⌘K pour effacer)"
                  className={classNames(
                    'w-full pl-8 pr-8 py-1.5 rounded-md text-sm',
                    'bg-fmc-elements-background-depth-2 border border-fmc-elements-borderColor',
                    'text-fmc-elements-textPrimary placeholder:text-fmc-elements-textTertiary',
                    'focus:outline-none focus:ring-2 focus:ring-fmc-elements-focus',
                    'transition-all',
                  )}
                ...
                <div className="absolute left-2.5 top-1/2 -translate-y-1/2">
                  <span className="i-ph:magnifying-glass text-fmc-elements-textTertiary" />
                </div>
                {providerSearchQuery && (
                  <button
                    type="button"
                    ...
                    className="absolute right-2.5 top-1/2 -translate-y-1/2 p-0.5 rounded hover:bg-fmc-elements-background-depth-3 transition-colors"
                    aria-label="Effacer la recherche"
                  >
                    <span className="i-ph:x text-fmc-elements-textTertiary text-xs" />
                  </button>
                )}
              </div>
            </div>
            ...
            <div
              className={classNames(
                ...
                '[&::-webkit-scrollbar-thumb]:bg-fmc-elements-borderColor',
                '[&::-webkit-scrollbar-thumb]:hover:bg-fmc-elements-borderColorHover',
                '[&::-webkit-scrollbar-thumb]:rounded-full',
                '[&::-webkit-scrollbar-track]:bg-fmc-elements-background-depth-2',
                '[&::-webkit-scrollbar-track]:rounded-full',
                'sm:[&::-webkit-scrollbar]:w-1.5 sm:[&::-webkit-scrollbar]:h-1.5',
                'sm:hover:[&::-webkit-scrollbar-thumb]:bg-fmc-elements-borderColor/50',
                'sm:hover:[&::-webkit-scrollbar-thumb:hover]:bg-fmc-elements-borderColor',
                'sm:[&::-webkit-scrollbar-track]:bg-transparent',
              )}
            >
              {filteredProviders.length === 0 ? (
                <div className="px-3 py-3 text-sm">
                  <div className="text-fmc-elements-textTertiary mb-1">
                    {debouncedProviderSearchQuery
                      ? `Aucun fournisseur ne correspond à "${debouncedProviderSearchQuery}"`
                      : 'Aucun fournisseur trouvé'}
                  </div>
                  {debouncedProviderSearchQuery && (
                    <div className="text-xs text-fmc-elements-textTertiary">
                      Essayez de rechercher des noms de fournisseurs comme "OpenAI", "Anthropic" ou "Google"
                    </div>
                  )}
                </div>
              ) : (
                  ...
                  <div
                    ...
                    className={classNames(
                      ...
                      provider?.name === providerOption.name || focusedProviderIndex === index
                        ? 'bg-fmc-elements-background-depth-2'
                        : undefined,
                      focusedProviderIndex === index ? 'ring-1 ring-inset ring-fmc-elements-focus' : undefined,
                    )}
                    onClick={(e) => {
                      e.stopPropagation();
      ...
      <div className="relative flex w-full min-w-[70%]" onKeyDown={handleModelKeyDown} ref={modelDropdownRef}>
        <div
          className={classNames(
            ...
            'focus-within:outline-none focus-within:ring-2 focus-within:ring-fmc-elements-focus',
            'transition-all cursor-pointer',
            isModelDropdownOpen ? 'ring-2 ring-fmc-elements-focus' : undefined,
          )}
        ...
        >
          <div className="flex items-center justify-between">
            <div className="truncate">{modelList.find((m) => m.name === model)?.label || 'Sélectionnez un modèle'}</div>
            <div
              className={classNames(
                'i-ph:caret-down w-4 h-4 text-fmc-elements-textSecondary opacity-75',
        ...
        {isModelDropdownOpen && (
          <div
            className="absolute z-10 w-full mt-1 py-1 rounded-lg border border-fmc-elements-borderColor bg-fmc-elements-background-depth-2 shadow-lg"
            role="listbox"
            id="model-listbox"
          >
            <div className="px-2 pb-2 space-y-2">
              ...
              {provider?.name === 'OpenRouter' && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={classNames(
                      ...
                      showFreeModelsOnly
                        ...
                        : 'bg-fmc-elements-background-depth-3 text-fmc-elements-textSecondary border border-fmc-elements-borderColor',
                    )}
                  >
                  ...
                  {showFreeModelsOnly && (
                    <span className="text-xs text-fmc-elements-textTertiary">
                      {filteredModels.length} {filteredModels.length !== 1 ? 'modèles gratuits' : 'modèle gratuit'}
                    </span>
                  )}
                </div>
              )}
              ...
              {debouncedModelSearchQuery && filteredModels.length > 0 && (
                <div className="text-xs text-fmc-elements-textTertiary px-1">
                  {filteredModels.length} {filteredModels.length !== 1 ? 'modèles trouvés' : 'modèle trouvé'}
                  {filteredModels.length > 5 && ' (affichage des meilleures correspondances)'}
                </div>
              )}
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="Rechercher des modèles... (⌘K pour effacer)"
                  className={classNames(
                    'w-full pl-8 pr-8 py-1.5 rounded-md text-sm',
                    'bg-fmc-elements-background-depth-2 border border-fmc-elements-borderColor',
                    'text-fmc-elements-textPrimary placeholder:text-fmc-elements-textTertiary',
            ...
            >
              {modelLoading === 'all' || modelLoading === provider?.name ? (
                <div className="px-3 py-3 text-sm">
                  <div className="flex items-center gap-2 text-fmc-elements-textTertiary">
                ...
                <div className="px-3 py-3 text-sm">
                  <div className="text-fmc-elements-textTertiary mb-1">
                    {debouncedModelSearchQuery
                      ? `Aucun modèle ne correspond à "${debouncedModelSearchQuery}"${showFreeModelsOnly ? ' (gratuit uniquement)' : ''}`
                      : showFreeModelsOnly
                        ? 'Aucun modèle gratuit disponible'
                        : provider?.name && LOCAL_PROVIDERS.includes(provider.name)
                          ? `Aucun modèle trouvé — est-ce que ${provider.name} fonctionne ?`
                          : 'Aucun modèle disponible'}
                  </div>
                  {!debouncedModelSearchQuery && provider?.name && LOCAL_PROVIDERS.includes(provider.name) && (
                    <div className="text-xs text-fmc-elements-textTertiary mt-1">
                      Assurez-vous que {provider?.name} est en cours d'exécution et possède au moins un modèle chargé.
                      {provider.name === 'Ollama' && ' Essayez : ollama pull llama3.2'}
                      {provider.name === 'LMStudio' && ' Chargez d\'abord un modèle dans LM Studio.'}
                    </div>
                  )}
                  {debouncedModelSearchQuery && (
                    <div className="text-xs text-fmc-elements-textTertiary">
                      Essayez de rechercher des noms de modèles, des tailles de contexte (ex: "128k", "1M") ou des capacités
                    </div>
                  )}
                  {showFreeModelsOnly && !debouncedModelSearchQuery && (
                    <div className="text-xs text-fmc-elements-textTertiary">
                      Essayez de désactiver le filtre "Modèles gratuits uniquement" pour voir tous les modèles disponibles
                    </div>
                  )}
                </div>
              ) : (
                  ...
                  <div
                    ...
                    className={classNames(
                      ...
                      model === modelOption.name || focusedModelIndex === index
                        ? 'bg-fmc-elements-background-depth-2'
                        : undefined,
                      focusedModelIndex === index ? 'ring-1 ring-inset ring-fmc-elements-focus' : undefined,
                    )}
                    onClick={(e) => {
                      e.stopPropagation();
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          {debouncedModelSearchQuery && (modelOption as any).searchScore > 70 && (
                            <span className="text-xs text-green-500 font-medium">
                              {(modelOption as any).searchScore.toFixed(0)}% de correspondance
                            </span>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-1 ml-2">
                        {isModelLikelyFree(modelOption, provider?.name) && (
                          <span className="i-ph:gift text-xs text-purple-400" title="Modèle gratuit" />
                        )}
                        {model === modelOption.name && (
                          <span className="i-ph:check text-xs text-green-500" title="Sélectionné" />
                        )}
                      </div>
                    </div>
                  </div>
