...
export function SupabaseChatAlert({ alert, clearAlert, postMessage }: Props) {
  ...
  const description = isConnected ? 'Exécuter la requête de base de données' : 'Connexion Supabase requise';
  ...
  const executeSupabaseAction = async (sql: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`La requête Supabase a échoué : ${errorData.error?.message || response.statusText}`);
      }
      ...
      postMessage(
        `*Erreur lors de l'exécution de la requête Supabase, veuillez corriger et renvoyer la requête*\n\`\`\`\n${error instanceof Error ? error.message : String(error)}\n\`\`\`\n`,
      );
    } finally {
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="max-w-chat rounded-lg border-l-2 border-l-[#098F5F] border border-fmc-elements-borderColor bg-fmc-elements-background-depth-2"
      >
        {/* Header */}
        ...
        <div className="px-4">
          {!isConnected ? (
            <div className="p-3 rounded-md bg-fmc-elements-background-depth-3">
              <span className="text-sm text-fmc-elements-textPrimary">
            ...
            <>
              <div
                className="flex items-center p-2 rounded-md bg-fmc-elements-background-depth-3 cursor-pointer"
                onClick={() => setIsCollapsed(!isCollapsed)}
              >
                <div className="i-ph:database text-fmc-elements-textPrimary mr-2"></div>
                <span className="text-sm text-fmc-elements-textPrimary flex-grow">
                  {description || 'Créer une table et configurer l\'auth'}
                </span>
                <div
                  className={`i-ph:caret-up text-fmc-elements-textPrimary transition-transform ${isCollapsed ? 'rotate-180' : ''}`}
                ></div>
              </div>

              {!isCollapsed && content && (
                <div className="mt-2 p-3 bg-fmc-elements-background-depth-4 rounded-md overflow-auto max-h-60 font-mono text-xs text-fmc-elements-textSecondary">
        ...
        {/* Message and Actions */}
        <div className="p-4">
          <p className="text-sm text-fmc-elements-textSecondary mb-4">{message}</p>
          ...
          <div className="flex gap-2">
            {showConnectButton ? (
              ...
              >
                {isExecuting ? 'Application...' : 'Appliquer les modifications'}
              </button>
            )}
            <button
              onClick={clearAlert}
