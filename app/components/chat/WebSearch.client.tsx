...
export function WebSearch({ onSearchResult, disabled = false }: WebSearchProps) {
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      toast.success('Contenu de l\'URL récupéré');
      setUrl('');
      setIsOpen(false);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Échec de la récupération de l\'URL');
    } finally {
  ...
  return (
    <div ref={containerRef} className="relative">
        <IconButton
          title="Récupérer le contenu de l'URL"
          disabled={disabled || isSearching}
          onClick={() => setIsOpen(!isOpen)}
          className="transition-all"
        >
          {isSearching ? (
            <div className="i-svg-spinners:90-ring-with-bg text-fmc-elements-loader-progress text-xl animate-spin" />
          ) : (
        ...
        {isOpen && (
          <div
            className={classNames(
              ...
              'rounded-lg border border-fmc-elements-borderColor bg-fmc-elements-background-depth-2 p-2 shadow-lg',
            )}
          >
            <input
              ref={inputRef}
              ...
              className={classNames(
                ...
                'bg-fmc-elements-background-depth-1 text-fmc-elements-textPrimary',
                'placeholder-fmc-elements-textTertiary',
            ...
            <button
              ...
              className={classNames(
                ...
                'bg-fmc-elements-button-primary-background text-fmc-elements-button-primary-text',
                'hover:bg-fmc-elements-button-primary-backgroundHover',
                'disabled:opacity-50 disabled:cursor-not-allowed',
              )}
            >
              {isSearching ? 'Récupération...' : 'Récupérer'}
            </button>
          </div>
        )}
    </div>
