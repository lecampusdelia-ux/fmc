...
export function GitLabRepositorySelector({ onClone, className }: GitLabRepositorySelectorProps) {
  ...
  const fetchRepositories = async (refresh = false) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData: any = await response.json().catch(() => ({ error: 'Échec de la récupération des dépôts' }));
        throw new Error(errorData.error || 'Échec de la récupération des dépôts');
      }
      ...
      setError(err instanceof Error ? err.message : 'Échec de la récupération des dépôts');
  ...
  if (!isConnected || !connection) {
    return (
      <div className="text-center p-8">
        <p className="text-fmc-elements-textSecondary mb-4">Veuillez d'abord vous connecter à GitLab pour parcourir les dépôts</p>
    ...
    return (
      <div className="text-center p-8">
        <div className="text-red-500 mb-4">
          ...
          <p className="font-medium">Échec du chargement des dépôts</p>
          <p className="text-sm text-fmc-elements-textSecondary mt-1">{error}</p>
        </div>
  ...
  if (isLoading && !repositories.length) {
    return (
      <div className="flex flex-col items-center justify-center p-8 space-y-4">
        <div className="animate-spin w-8 h-8 border-2 border-fmc-elements-borderColorActive border-t-transparent rounded-full" />
        <p className="text-sm text-fmc-elements-textSecondary">Chargement des dépôts...</p>
      </div>
    );
  }
  ...
  if (!repositories.length && !isLoading) {
    return (
      <div className="text-center p-8">
        <GitBranch className="w-12 h-12 text-fmc-elements-textTertiary mx-auto mb-4" />
        <p className="text-fmc-elements-textSecondary mb-4">Aucun dépôt trouvé</p>
        <Button variant="outline" onClick={handleRefresh} disabled={isRefreshing}>
          <RefreshCw className={classNames('w-4 h-4 mr-2', { 'animate-spin': isRefreshing })} />
          Actualiser
        </Button>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-fmc-elements-textPrimary">Sélectionnez le dépôt à cloner</h3>
          <p className="text-sm text-fmc-elements-textSecondary">
            {filteredRepositories.length} sur {repositories.length} dépôts
          </p>
        </div>
        <Button
      ...
      {error && repositories.length > 0 && (
        <div className="p-3 rounded-lg bg-yellow-50 border border-yellow-200 dark:bg-yellow-900/20 dark:border-yellow-700">
          <p className="text-sm text-yellow-800 dark:text-yellow-200">Attention : {error}. Affichage des données en cache.</p>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-fmc-elements-textTertiary" />
          <input
            type="text"
            placeholder="Rechercher des dépôts..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-4 py-2 rounded-lg bg-fmc-elements-background-depth-1 border border-fmc-elements-borderColor text-fmc-elements-textPrimary placeholder-fmc-elements-textTertiary focus:outline-none focus:ring-1 focus:ring-fmc-elements-borderColorActive"
          />
        </div>

        {/* Sort */}
        <div className="flex items-center gap-2">
          <Calendar className="w-4 h-4 text-fmc-elements-textTertiary" />
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as SortOption)}
            className="px-3 py-2 rounded-lg bg-fmc-elements-background-depth-1 border border-fmc-elements-borderColor text-fmc-elements-textPrimary text-sm focus:outline-none focus:ring-1 focus:ring-fmc-elements-borderColorActive"
          >
            <option value="updated">Récemment mis à jour</option>
            <option value="stars">Les plus étoilés</option>
            <option value="name">Nom (A-Z)</option>
            <option value="created">Récemment créés</option>
          </select>
        </div>

        {/* Filter */}
        <div className="flex items-center gap-2">
          <Filter className="w-4 h-4 text-fmc-elements-textTertiary" />
          <select
            value={filterBy}
          ...
          >
            ...
            <option value="owned">Dépôts possédés</option>
            <option value="member">Dépôts membres</option>
          </select>
        </div>
      </div>
      ...
      {currentRepositories.length > 0 ? (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between pt-4 border-t border-fmc-elements-borderColor">
              <div className="text-sm text-fmc-elements-textSecondary">
                ...
                {Math.min(startIndex + REPOS_PER_PAGE, filteredRepositories.length)} sur {filteredRepositories.length}{' '}
                dépôts
              </div>
              <div className="flex items-center gap-2">
                <Button
        ...
        <div className="text-center py-8">
          <p className="text-fmc-elements-textSecondary">Aucun dépôt trouvé correspondant à vos critères de recherche.</p>
        </div>
      )}
