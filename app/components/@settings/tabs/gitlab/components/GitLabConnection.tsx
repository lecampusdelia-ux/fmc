...
export default function GitLabConnection({ connectionTest, onTestConnection }: GitLabConnectionProps) {
  ...
  const handleDisconnect = () => {
    disconnect();
    toast.success('Déconnecté de GitLab');
  };

  return (
    <motion.div
      className="bg-fmc-elements-background border border-fmc-elements-borderColor rounded-lg"
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-medium text-fmc-elements-textPrimary">Connexion GitLab</h3>
          </div>
        </div>

        {!isConnected && (
          <div className="text-xs text-fmc-elements-textSecondary bg-fmc-elements-background-depth-1 p-3 rounded-lg mb-4">
            <p className="flex items-center gap-1 mb-1">
              ...
              <code className="px-1 py-0.5 bg-fmc-elements-background-depth-2 rounded">VITE_GITLAB_ACCESS_TOKEN</code>{' '}
              pour vous connecter automatiquement.
            </p>
            <p>
              Pour les instances GitLab auto-hébergées, définissez également{' '}
              <code className="px-1 py-0.5 bg-fmc-elements-background-depth-2 rounded">
                VITE_GITLAB_URL=https://votre-instance-gitlab.com
              </code>
            </p>
          </div>
        )}
        ...
        <form onSubmit={handleConnect}>
          <div className="grid grid-cols-1 gap-4">
            <div>
              <label className="block text-sm text-fmc-elements-textSecondary mb-2">GitLab URL</label>
              <input
                type="text"
                value={gitlabUrl}
                ...
                className={classNames(
                  ...
                  'bg-fmc-elements-background-depth-1',
