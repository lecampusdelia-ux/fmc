  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      if (isValid) {
        setConnectionTest({
          status: 'success',
          message: `Connecté avec succès en tant que ${connection.user.username}`,
          timestamp: Date.now(),
        });
      } else {
        setConnectionTest({
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-2">
          <GitLabLogo />
          <h2 className="text-lg font-medium text-fmc-elements-textPrimary">Intégration GitLab</h2>
        </div>
        <div className="flex items-center justify-center p-4">
          <div className="flex items-center gap-2">
            <div className="i-ph:spinner-gap-bold animate-spin w-4 h-4" />
            <span className="text-fmc-elements-textSecondary">Chargement...</span>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="space-y-6">
        ...
        <p className="text-sm text-fmc-elements-textSecondary">
          Connectez votre compte GitLab pour activer les fonctionnalités avancées de gestion de dépôts, les statistiques et une intégration fluide.
        </p>
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-fmc-elements-textSecondary dark:text-fmc-elements-textSecondary">
        Gérez votre intégration GitLab avec des fonctionnalités de dépôt avancées et des statistiques complètes
      </p>
      ...
      {connection?.user && (
        ...
        >
          <div className="flex items-center gap-4 p-4 bg-fmc-elements-background-depth-1 rounded-lg">
            <div className="w-12 h-12 rounded-full border-2 border-fmc-elements-item-contentAccent flex items-center justify-center bg-fmc-elements-background-depth-2 overflow-hidden">
              ...
              connection.user.avatar_url !== '' ? (
                ...
                <div className="w-full h-full rounded-full bg-fmc-elements-item-contentAccent flex items-center justify-center text-white font-semibold text-sm">
            ...
            <div>
              ...
              <p className="text-sm text-fmc-elements-textSecondary">{connection.user?.username}</p>
            </div>
          </div>
        </motion.div>
      )}
      ...
      {connection?.stats && (
        <motion.div
          ...
          className="border-t border-fmc-elements-borderColor pt-6"
        >
          <h3 className="text-base font-medium text-fmc-elements-textPrimary mb-4">Statistiques</h3>
          <StatsDisplay
