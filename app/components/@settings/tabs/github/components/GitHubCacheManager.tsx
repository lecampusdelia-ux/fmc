  ...
  return (
    <div
      className={classNames(
        'space-y-4 p-4 bg-fmc-elements-background-depth-1 border border-fmc-elements-borderColor rounded-lg',
        className,
      )}
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Database className="w-4 h-4 text-fmc-elements-item-contentAccent" />
          <h3 className="text-sm font-medium text-fmc-elements-textPrimary">Gestion du cache GitHub</h3>
        </div>
      ...
      {showStats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-fmc-elements-background-depth-2 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-1">
              <HardDrive className="w-3 h-3 text-fmc-elements-textSecondary" />
              <span className="text-xs font-medium text-fmc-elements-textSecondary">Taille totale</span>
            </div>
            <p className="text-sm font-semibold text-fmc-elements-textPrimary">
          ...
          <div className="bg-fmc-elements-background-depth-2 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-1">
              <Database className="w-3 h-3 text-fmc-elements-textSecondary" />
              <span className="text-xs font-medium text-fmc-elements-textSecondary">Entrées</span>
            </div>
            <p className="text-sm font-semibold text-fmc-elements-textPrimary">{cacheStats.totalEntries}</p>
          </div>

          <div className="bg-fmc-elements-background-depth-2 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-1">
              <Clock className="w-3 h-3 text-fmc-elements-textSecondary" />
              <span className="text-xs font-medium text-fmc-elements-textSecondary">La plus ancienne</span>
            </div>
          ...
          <div className="bg-fmc-elements-background-depth-2 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-1">
              <CheckCircle className="w-3 h-3 text-fmc-elements-textSecondary" />
              <span className="text-xs font-medium text-fmc-elements-textSecondary">État</span>
            </div>
            <p className="text-xs text-green-600 dark:text-green-400">
              {cacheStats.totalEntries > 0 ? 'Actif' : 'Vide'}
            </p>
          </div>
        </div>
      )}
      ...
      {cacheEntries.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-xs font-medium text-fmc-elements-textSecondary">
            Entrées du cache ({cacheEntries.length})
          </h4>
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {cacheEntries.map((entry) => (
              <div
                key={entry.key}
                className="flex items-center justify-between p-2 bg-fmc-elements-background-depth-2 rounded border border-fmc-elements-borderColor"
              >
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-fmc-elements-textPrimary truncate">
      ...
      <div className="flex flex-wrap gap-2 pt-2 border-t border-fmc-elements-borderColor">
        <Button
          variant="outline"
          size="sm"
        ...
        >
          ...
          <span className="text-xs">Effacer expirés</span>
        </Button>
        ...
        >
          ...
          <span className="text-xs">Compacter</span>
        </Button>
      ...
      {lastClearTime && (
        <div className="flex items-center gap-2 p-2 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded text-xs text-green-700 dark:text-green-400">
          ...
          <span>Cache effacé avec succès à {new Date(lastClearTime).toLocaleTimeString()}</span>
        </div>
      )}
    </div>
  );
