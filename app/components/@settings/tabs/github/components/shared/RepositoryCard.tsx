...
export function RepositoryCard({
  ...
  const formatTimeAgo = () => {
    ...
    if (daysSinceUpdate < 7) {
      return `Il y a ${daysSinceUpdate} jours`;
    }

    if (daysSinceUpdate < 30) {
      return `Il y a ${Math.floor(daysSinceUpdate / 7)} semaines`;
    }
    ...
    return (
      <button
        ...
        className={classNames(
          'w-full text-left p-3 rounded-lg border border-fmc-elements-borderColor hover:border-fmc-elements-borderColorActive hover:bg-fmc-elements-background-depth-1 transition-all duration-200',
          className,
        )}
      >
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-2">
            <h4 className="text-sm font-medium text-fmc-elements-textPrimary">{repository.name}</h4>
            {repository.private && <Lock className="w-3 h-3 text-fmc-elements-textTertiary" />}
            {repository.fork && <GitFork className="w-3 h-3 text-fmc-elements-textTertiary" />}
            {repository.archived && <Archive className="w-3 h-3 text-fmc-elements-textTertiary" />}
          </div>
        ...
        {repository.description && (
          <p className="text-xs text-fmc-elements-textSecondary mb-2 line-clamp-2">{repository.description}</p>
        )}

        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 text-xs text-fmc-elements-textTertiary">
            {repository.language && (
              <span className="flex items-center gap-1">
          ...
          <span className="flex items-center gap-1 text-xs text-fmc-elements-textTertiary">
  ...
  const interactiveProps = onSelect
    ? {
      onClick: onSelect,
      className: classNames(
        'group cursor-pointer hover:border-fmc-elements-borderColorActive dark:hover:border-fmc-elements-borderColorActive transition-all duration-200',
        className,
      ),
    }
    : { className };
  ...
  return (
    <Component
      {...interactiveProps}
      className={classNames(
        'block p-4 rounded-lg bg-fmc-elements-background-depth-1 dark:bg-fmc-elements-background-depth-1 border border-fmc-elements-borderColor dark:border-fmc-elements-borderColor relative',
    ...
    >
      ...
      {variant === 'detailed' && (
        <div
          ...
          title={`Santé du dépôt : ${getHealthTitle()}`}
        />
      )}
      ...
      <div className="space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            <GitBranch className="w-4 h-4 text-fmc-elements-icon-tertiary" />
            <h5
              className={classNames(
                'text-sm font-medium text-fmc-elements-textPrimary',
                onSelect && 'group-hover:text-fmc-elements-item-contentAccent transition-colors',
              )}
            >
              {repository.name}
            </h5>
            {repository.fork && (
              <span title="Dépôt forké">
                <GitFork className="w-3 h-3 text-fmc-elements-textTertiary" />
              </span>
            )}
            {repository.archived && (
              <span title="Dépôt archivé">
                <Archive className="w-3 h-3 text-fmc-elements-textTertiary" />
              </span>
            )}
          </div>
          <div className="flex items-center gap-3 text-xs text-fmc-elements-textSecondary">
            <span className="flex items-center gap-1" title="Stars">
              <Star className="w-3.5 h-3.5 text-fmc-elements-icon-warning" />
              {repository.stargazers_count.toLocaleString()}
            </span>
            <span className="flex items-center gap-1" title="Forks">
              <GitFork className="w-3.5 h-3.5 text-fmc-elements-icon-info" />
              {repository.forks_count.toLocaleString()}
            </span>
            {showExtendedMetrics && repository.issues_count !== undefined && (
              <span className="flex items-center gap-1" title="Open Issues">
                <Circle className="w-3.5 h-3.5 text-fmc-elements-icon-error" />
            ...
            {showExtendedMetrics && repository.pull_requests_count !== undefined && (
              <span className="flex items-center gap-1" title="Pull Requests">
                <GitPullRequest className="w-3.5 h-3.5 text-fmc-elements-icon-success" />
        ...
        <div className="space-y-2">
          {repository.description && (
            <p className="text-xs text-fmc-elements-textSecondary line-clamp-2">{repository.description}</p>
          )}

          {/* Repository metrics bar */}
          <div className="flex items-center gap-2 text-xs">
            {repository.license && (
              <span className="px-2 py-0.5 rounded-full bg-fmc-elements-background-depth-2 text-fmc-elements-textTertiary">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 text-xs text-fmc-elements-textSecondary">
            ...
            {repository.size && (
              <span className="flex items-center gap-1" title="Taille">
                <Database className="w-3.5 h-3.5" />
                {(repository.size / 1024).toFixed(1)} Mo
              </span>
            )}
            ...
            {onSelect && (
              <span
                className={classNames(
                  ...
                  'group-hover:text-fmc-elements-item-contentAccent',
                )}
              >
