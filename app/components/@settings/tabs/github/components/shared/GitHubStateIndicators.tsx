...
export function LoadingState({ message = 'Chargement...', size = 'md', className = '' }: LoadingStateProps) {
  const sizeClasses = {
    sm: 'w-4 h-4',
    md: 'w-6 h-6',
    lg: 'w-8 h-8',
  ...
  return (
    <div
      className={classNames(
        'flex flex-col items-center justify-center py-8 text-fmc-elements-textSecondary',
        className,
      )}
    >
      <Loader2 className={classNames('animate-spin mb-2', sizeClasses[size])} />
      <p className={classNames('text-fmc-elements-textSecondary', textSizeClasses[size])}>{message}</p>
    </div>
  );
}
...
export function ErrorState({
  ...
  return (
    <div className={classNames('flex flex-col items-center justify-center py-8 text-center', className)}>
      ...
      <h3 className={classNames('font-medium text-fmc-elements-textPrimary mb-1', textSizeClasses[size])}>{title}</h3>
      <p className={classNames('text-fmc-elements-textSecondary mb-4', textSizeClasses[size])}>{message}</p>
      {onRetry && (
        <button
          onClick={onRetry}
          className="px-4 py-2 bg-fmc-elements-item-contentAccent text-white rounded-lg hover:bg-fmc-elements-item-contentAccent/90 transition-colors"
        >
          {retryLabel}
        </button>
      )}
...
export function GitHubConnectionRequired({ onConnect, className = '' }: GitHubConnectionRequiredProps) {
  return (
    <div className={classNames('flex flex-col items-center justify-center py-12 text-center', className)}>
      <Github className="w-12 h-12 text-fmc-elements-textTertiary mb-4" />
      <h3 className="text-lg font-medium text-fmc-elements-textPrimary mb-2">Connexion GitHub requise</h3>
      <p className="text-sm text-fmc-elements-textSecondary mb-6 max-w-md">
        Veuillez connecter votre compte GitHub pour accéder à cette fonctionnalité. Vous pourrez parcourir les dépôts, pousser du code et gérer votre intégration GitHub.
      </p>
      {onConnect && (
        <button
          onClick={onConnect}
          className="px-6 py-3 bg-fmc-elements-item-contentAccent text-white rounded-lg hover:bg-fmc-elements-item-contentAccent/90 transition-colors flex items-center gap-2"
        >
