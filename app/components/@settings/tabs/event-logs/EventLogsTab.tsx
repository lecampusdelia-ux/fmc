      ...
      return (
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
            <span>Modèle : {details.model}</span>
            <span>•</span>
            <span>Tokens : {details.totalTokens}</span>
            <span>•</span>
            <span>Durée : {details.duration}ms</span>
          </div>
          {details.prompt && (
      ...
      return (
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
            ...
            <span>•</span>
            <span>État : {details.statusCode}</span>
            <span>•</span>
            <span>Durée : {details.duration}ms</span>
          </div>
...
export function EventLogsTab() {
  ...
  const exportAsJSON = () => {
    try {
      ...
      a.href = url;
      a.download = `fmc-event-logs-${new Date().toISOString()}.json`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      toast.success('Journaux d\'événements exportés avec succès en JSON');
    } catch (error) {
      console.error('Failed to export JSON:', error);
      toast.error('Échec de l\'exportation des journaux d\'événements en JSON');
    }
  };
  ...
  const exportAsCSV = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `fmc-event-logs-${new Date().toISOString()}.csv`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      toast.success('Journaux d\'événements exportés avec succès en CSV');
    } catch (error) {
      console.error('Failed to export CSV:', error);
      toast.error('Échec de l\'exportation des journaux d\'événements en CSV');
    }
  };
  ...
  const exportAsPDF = () => {
    try {
      ...
      doc.setFontSize(24);
      doc.setFont('helvetica', 'bold');
      doc.text('Rapport des journaux d\'événements', margin, 35);

      // Add subtitle with fmc
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('fmc - Plateforme de développement d\'IA', margin, 45);
      yPos = 70;

      // Add report summary section
      addSectionHeader('Résumé du rapport');
      ...
      const summaryItems = [
        { label: 'Généré le', value: new Date().toLocaleString() },
        { label: 'Total des journaux', value: filteredLogs.length.toString() },
        { label: 'Filtre appliqué', value: selectedLevel === 'all' ? 'Tous les types' : selectedLevel },
        { label: 'Requête de recherche', value: searchQuery || 'Aucune' },
        { label: 'Format de l\'heure', value: use24Hour ? '24 heures' : '12 heures' },
      ];
      ...
      addSectionHeader('Statistiques des journaux');
      ...
      const leftStats = [
        { label: 'Journaux d\'erreurs', value: stats.error, color: '#DC2626' },
        { label: 'Journaux d\'avertissements', value: stats.warning, color: '#F59E0B' },
        { label: 'Journaux d\'infos', value: stats.info, color: '#3B82F6' },
      ];

      const rightStats = [
        { label: 'Journaux de débogage', value: stats.debug, color: '#6B7280' },
        { label: 'Journaux LLM', value: stats.provider, color: '#10B981' },
        { label: 'Journaux API', value: stats.api, color: '#3B82F6' },
      ];
      ...
      // Add logs section
      addSectionHeader('Journaux d\'événements');
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Add footer text
        doc.text('Generated by fmc', margin, doc.internal.pageSize.getHeight() - 10);
      ...
      // Save the PDF
      doc.save(`fmc-event-logs-${new Date().toISOString()}.pdf`);
      toast.success('Journaux d\'événements exportés avec succès en PDF');
    } catch (error) {
      console.error('Failed to export PDF:', error);
      toast.error('Échec de l\'exportation des journaux d\'événements en PDF');
    }
  };
  ...
  const exportAsText = () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `fmc-event-logs-${new Date().toISOString()}.txt`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      toast.success('Journaux d\'événements exportés avec succès en fichier texte');
    } catch (error) {
      console.error('Failed to export text file:', error);
      toast.error('Échec de l\'exportation des journaux d\'événements en fichier texte');
    }
  };
              ...
              {exportFormats.map((format) => (
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-fmc-elements-textSecondary mt-0.5">
                      {format.id === 'json' && 'Exporter en tant que fichier JSON structuré'}
                      {format.id === 'csv' && 'Exporter en tant que feuille de calcul CSV'}
                      {format.id === 'pdf' && 'Exporter en tant que document PDF formaté'}
                      {format.id === 'txt' && 'Exporter en tant que fichier texte formaté'}
                    </div>
                  </div>
                </button>
              ))}
      ...
      <div className="flex items-center justify-between">
        <DropdownMenu.Root open={showLevelFilter} onOpenChange={setShowLevelFilter}>
          <DropdownMenu.Trigger asChild>
            ...
            >
              ...
              />
              {selectedLevelOption?.label || 'Tous les types'}
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-sm text-gray-500 dark:text-gray-400">Afficher l'horodatage</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Switch
              ...
              className="data-[state=checked]:bg-purple-500"
            />
            <span className="text-sm text-gray-500 dark:text-gray-400">Format 24h</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Switch
              ...
              className="data-[state=checked]:bg-purple-500"
            />
            <span className="text-sm text-gray-500 dark:text-gray-400">Développer auto.</span>
          </div>
