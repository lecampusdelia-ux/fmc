      ...
      <div className="p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={classNames(feature.icon, 'w-5 h-5 text-fmc-elements-textSecondary')} />
            <div className="flex items-center gap-2">
              <h4 className="font-medium text-fmc-elements-textPrimary">{feature.title}</h4>
              {feature.beta && (
        ...
        <p className="mt-2 text-sm text-fmc-elements-textSecondary">{feature.description}</p>
        {feature.tooltip && <p className="mt-1 text-xs text-fmc-elements-textTertiary">{feature.tooltip}</p>}
      </div>
    </motion.div>
  ),
);
  ...
  ({
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="text-lg font-medium text-fmc-elements-textPrimary">{title}</h3>
          <p className="text-sm text-fmc-elements-textSecondary">{description}</p>
        </div>
      </div>
  ...
  const features = {
    stable: [
      ...
      },
      {
        ...
        description: 'Sélectionner automatiquement le modèle de démarrage',
        icon: 'i-ph:selection',
      ...
      },
      {
        ...
        description: 'Optimiser le contexte pour de meilleures réponses',
        icon: 'i-ph:brain',
      ...
      },
      {
        ...
        description: "Activer la journalisation détaillée des événements et l'historique",
    ...
    <div className="flex flex-col gap-8">
      ...
      {features.beta.length > 0 && (
        <FeatureSection
          ...
          icon="i-ph:test-tube"
          description="Nouvelles fonctionnalités prêtes à être testées mais pouvant présenter quelques imperfections"
          onToggleFeature={handleToggleFeature}
        />
      )}
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-fmc-elements-textSecondary mt-0.5">
              Choisissez un prompt dans la bibliothèque pour l'utiliser comme prompt système
            </p>
          </div>
          <select
            value={promptId}
