...
const PROVIDER_DESCRIPTIONS: Partial<Record<ProviderName, string>> = {
  Anthropic: 'Accédez à Claude et aux autres modèles d\'Anthropic',
  Github: 'Utilisez les modèles OpenAI hébergés via l\'infrastructure GitHub',
  OpenAI: 'Utilisez GPT-4, GPT-3.5 et d\'autres modèles OpenAI',
};
...
const CloudProvidersTab = () => {
  ...
  const handleToggleProvider = useCallback(
    (provider: IProviderConfig, enabled: boolean) => {
      ...
      if (enabled) {
        ...
        toast.success(`${provider.name} activé`);
      } else {
        logStore.logProvider(`Provider ${provider.name} disabled`, { provider: provider.name });
        toast.success(`${provider.name} désactivé`);
      }
    },
    [settings],
  );
  ...
  const handleUpdateBaseUrl = useCallback(
    (provider: IProviderConfig, baseUrl: string) => {
      ...
      toast.success(`URL de base de ${provider.name} mise à jour`);
      ...
      >
        <div className="flex items-center justify-between gap-4 mt-8 mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="text-md font-medium text-fmc-elements-textPrimary">Fournisseurs Cloud</h4>
              <p className="text-sm text-fmc-elements-textSecondary">Connectez-vous aux modèles et services d'IA basés sur le cloud</p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-sm text-fmc-elements-textSecondary">Tout activer</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredProviders.map((provider, index) => (
            <motion.div
              key={provider.name}
              className={classNames(
                'rounded-lg border bg-fmc-elements-background text-fmc-elements-textPrimary shadow-sm',
              ...
              <div className="flex items-start gap-4 p-4">
                <motion.div
                  className={classNames(
                    ...
                    'bg-fmc-elements-background-depth-3 group-hover:bg-fmc-elements-background-depth-4',
                    'transition-all duration-200',
                    provider.settings.enabled ? 'text-purple-500' : 'text-fmc-elements-textSecondary',
                  )}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-4 mb-2">
                    <div>
                      <h4 className="text-sm font-medium text-fmc-elements-textPrimary group-hover:text-purple-500 transition-colors">
                        {provider.name}
                      </h4>
                      <p className="text-xs text-fmc-elements-textSecondary mt-0.5">
                        {PROVIDER_DESCRIPTIONS[provider.name as keyof typeof PROVIDER_DESCRIPTIONS] ||
                          (URL_CONFIGURABLE_PROVIDERS.includes(provider.name)
                            ? 'Configurer un point de terminaison personnalisé'
                            : 'Intégration standard de fournisseur d\'IA')}
                      </p>
                    </div>
                    <Switch
                      ...
                      <div className="flex items-center gap-2 mt-4">
                        {editingProvider === provider.name ? (
                          <input
                            ...
                            placeholder={`Entrez l'URL de base pour ${provider.name}`}
                            className={classNames(
                              'flex-1 px-3 py-1.5 rounded-lg text-sm',
                              'bg-fmc-elements-background-depth-3 border border-fmc-elements-borderColor',
                          ...
                          >
                            <div className="flex items-center gap-2 text-fmc-elements-textSecondary">
