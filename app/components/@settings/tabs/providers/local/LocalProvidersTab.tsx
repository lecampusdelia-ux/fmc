...
export default function LocalProvidersTab() {
  ...
  const handleToggleProvider = useCallback(
    (provider: IProviderConfig, enabled: boolean) => {
      ...
      toast(`${provider.name} ${enabled ? 'activé' : 'désactivé'}`);
    },
  ...
  const handleUpdateBaseUrl = useCallback(
    (provider: IProviderConfig, newBaseUrl: string) => {
      ...
      toast(`URL de base de ${provider.name} mise à jour`);
    },
    [updateProviderSettings, toast],
  );
  ...
  const handleUpdateOllamaModel = async (modelName: string) => {
    try {
      ...
      toast(`Mise à jour réussie de ${modelName}`);
    } catch {
      setOllamaModels((prev) =>
        prev.map((m) => (m.name === modelName ? { ...m, status: 'error', progress: undefined } : m)),
      );
      toast(`Échec de la mise à jour de ${modelName}`, { type: 'error' });
    }
  };

  const handleDeleteOllamaModel = async (modelName: string) => {
    if (!window.confirm(`Êtes-vous sûr de vouloir supprimer ${modelName} ?`)) {
      return;
    }
    ...
    try {
      ...
      toast(`${modelName} supprimé`);
    } catch {
      toast(`Échec de la suppression de ${modelName}`, { type: 'error' });
    }
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-fmc-elements-textPrimary">Fournisseurs d'IA locaux</h2>
              <p className="text-sm text-fmc-elements-textSecondary">Configurez et gérez vos modèles d'IA locaux</p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-fmc-elements-textSecondary">Tout activer</span>
              <Switch
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className="bg-fmc-elements-background-depth-2 hover:bg-fmc-elements-background-depth-3 border-fmc-elements-borderColor hover:border-purple-500/30 transition-all duration-200 gap-2"
              >
          ...
          {filteredProviders.map((provider) => (
            <div key={provider.name} className="space-y-4">
              ...
              {provider.name === 'Ollama' && provider.settings.enabled && (
                <Card className="mt-4 bg-fmc-elements-background-depth-2">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <PackageOpen className="w-5 h-5 text-purple-500" />
                        <h3 className="text-lg font-semibold text-fmc-elements-textPrimary">Modèles installés</h3>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
                        ...
                        className="bg-transparent hover:bg-fmc-elements-background-depth-2"
                      >
                        {isLoadingModels ? (
                  ...
                  <CardContent className="space-y-4">
                    {isLoadingModels ? (
                      ...
                      <div className="text-center py-8">
                        <PackageOpen className="w-16 h-16 mx-auto text-fmc-elements-textTertiary mb-4" />
                        <h3 className="text-lg font-medium text-fmc-elements-textPrimary mb-2">Aucun modèle installé</h3>
                        <p className="text-sm text-fmc-elements-textSecondary mb-4">
                          Visitez{' '}
                          <a
                        ...
                        >
                          ...
                          >
                            ...
                            <span className="flex-1 text-center font-medium">Parcourir les modèles</span>
                          </a>
                        </Button>
                      </div>
                    ) : (
                ...
                <Card className="mt-4 bg-fmc-elements-background-depth-2">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-lg font-semibold text-fmc-elements-textPrimary">Modèles disponibles</h3>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
                  ...
                  <CardContent className="space-y-4">
                    {isLoadingLMStudioModels ? (
                      ...
                      <div className="text-center py-8">
                        <Monitor className="w-16 h-16 mx-auto text-fmc-elements-textTertiary mb-4" />
                        <h3 className="text-lg font-medium text-fmc-elements-textPrimary mb-2">Aucun modèle disponible</h3>
                        <p className="text-sm text-fmc-elements-textSecondary mb-4">
                          Assurez-vous que LM Studio fonctionne avec le serveur local démarré et CORS activé.
                        </p>
                        <Button
                          variant="outline"
                          size="sm"
                        ...
                        >
                          ...
                          >
                            <ExternalLink className="w-4 h-4 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-300 flex-shrink-0" />
                            <span className="flex-1 text-center font-medium">Obtenir LM Studio</span>
                          </a>
                        </Button>
                      </div>
                    ) : (
                      <div className="grid gap-4">
                        {lmStudioModels.map((model) => (
                          <Card key={model.id} className="bg-fmc-elements-background-depth-3">
                            <CardContent className="p-4">
                              <div className="space-y-2">
                                <div className="flex items-center gap-2">
                                  <h4 className="text-sm font-medium text-fmc-elements-textPrimary font-mono">
                                    {model.id}
                                  </h4>
                                ...
                                <div className="flex items-center gap-4 text-xs text-fmc-elements-textSecondary">
                                  ...
                                  <div className="flex items-center gap-1">
                                    ...
                                    <span>Propriétaire : {model.owned_by}</span>
                                  </div>
                                  {model.created && (
                                    <div className="flex items-center gap-1">
                                      <Activity className="w-3 h-3" />
                                      <span>Créé le : {new Date(model.created * 1000).toLocaleDateString()}</span>
                                    </div>
                                  )}
                                </div>
                              </div>
        ...
        {filteredProviders.length === 0 && (
          <Card className="bg-fmc-elements-background-depth-2">
            <CardContent className="p-8 text-center">
              <Server className="w-16 h-16 mx-auto text-fmc-elements-textTertiary mb-4" />
              <h3 className="text-lg font-medium text-fmc-elements-textPrimary mb-2">Aucun fournisseur local disponible</h3>
              <p className="text-sm text-fmc-elements-textSecondary">
                Les fournisseurs locaux apparaîtront ici lorsqu'ils seront configurés dans le système.
              </p>
            </CardContent>
          </Card>
        )}
