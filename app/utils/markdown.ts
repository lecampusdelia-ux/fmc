  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__fmcThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__fmcArtifact__', '__fmcThought__', '__fmcQuickAction', '__fmcSelectedElement__'],

      // ['className', '__fmcThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__fmcArtifact__', '__fmcThought__', '__fmcQuickAction'],
    ],
  },
  strip: [],
};
