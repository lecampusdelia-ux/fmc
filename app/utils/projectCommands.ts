...
export function createCommandsMessage(commands: ProjectCommands): Message | null {
  ...
  if (commands.setupCommand) {
    commandString += `
<fmcAction type="shell">${commands.setupCommand}</fmcAction>`;
  }

  if (commands.startCommand) {
    commandString += `
<fmcAction type="start">${commands.startCommand}</fmcAction>
`;
  }
...
<fmcArtifact id="project-setup" title="Project Setup">
${commandString}
</fmcArtifact>`,
    id: generateId(),
    createdAt: new Date(),
...
export function escapeFmcArtifactTags(input: string) {
  // Regular expression to match fmcArtifact tags and their content
  const regex = /(<fmcArtifact[^>]*>)([\s\S]*?)(<\/fmcArtifact>)/g;
...
export function escapeFmcActionTags(input: string) {
  // Regular expression to match fmcAction tags and their content
  const regex = /(<fmcAction[^>]*>)([\s\S]*?)(<\/fmcAction>)/g;
...
export function escapeFmcTags(input: string) {
  return escapeFmcArtifactTags(escapeFmcActionTags(input));
}
