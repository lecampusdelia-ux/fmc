...
export class FmcShell {
  ...
  async init(webcontainer: WebContainer, terminal: ITerminal) {
    ...
    const { process, commandStream, expoUrlStream } = await this.newFmcShellProcess(webcontainer, terminal);
  ...
  async newFmcShellProcess(webcontainer: WebContainer, terminal: ITerminal) {
    const args: string[] = [];
    const process = await webcontainer.spawn('/bin/jsh', ['--osc', ...args], {
      terminal: {
        cols: terminal.cols ?? 80,
  ...
  async waitTillOscCode(waitCode: string) {
    ...
    while (true) {
      ...
      // We use a global match to find ALL OSC codes in the current chunk
      const oscMatches = text.matchAll(/\x1b\]654;([^\x07=]+)=?((-?\d+):(\d+))?\x07/g);
...
export function newFmcShellProcess() {
  return new FmcShell();
}
