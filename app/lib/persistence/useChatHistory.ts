            ...
            if (startingIdx > 0) {
              ...
              filteredMessages = [
                ...
                },
                {
                  ...
                  content: `FMC a restauré votre discussion à partir d'un instantané. Vous pouvez revenir à ce message pour charger l'historique complet.
                  <fmcArtifact id="restored-project-setup" title="Projet Restauré & Configuration" type="bundled">
                  ${Object.entries(snapshot?.files || {})
                      .map(([key, value]) => {
                        if (value?.type === 'file') {
                          return `
                      <fmcAction type="file" filePath="${key}">
${value.content}
                      </fmcAction>
                      `;
                        } else {
