...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  ...
  private _hasDetectedArtifacts(input: string): boolean {
    return input.includes('<fmcArtifact') || input.includes('</fmcArtifact>') || input.includes('<fmcArtifact');
  }

  private _detectAndWrapCodeBlocks(messageId: string, input: string): string {
    // OPTIMIZATION: If there's an unclosed code block, wait for it to close before running heavy regexes
    ...
    const patterns = [
      ...
      {
        regex:
          /```(?:json|jsx?|tsx?|html?|vue|svelte)\n(\{[\s\S]*?"(?:name|version|scripts|dependencies|devDependencies)"[\s\S]*?\}|<[a-zA-Z][^>]*>[\s\S]*?)```/gi,
  ...
  private _wrapInArtifact(artifactId: string, filePath: string, content: string): string {
    ...
    return `<fmcArtifact id="${artifactId}" title="${title}" type="bundled">
<fmcAction type="file" filePath="${filePath}">
${content}
</fmcAction>
</fmcArtifact>`;
  }
  ...
  private _wrapInShellAction(content: string, messageId: string): string {
    ...
    // Detect if this is a development server start command
    const isStartCommand = /^(npm|yarn|pnpm)\s+(run\s+)?(start|dev|preview)/.test(content.trim());
    const actionType = isStartCommand ? 'start' : 'shell';
    const title = isStartCommand ? 'Start Server' : 'Shell Command';

    return `<fmcArtifact id="${artifactId}" title="${title}" type="${actionType}">
<fmcAction type="${actionType}">
${content.trim()}
</fmcAction>
</fmcArtifact>`;
  }
