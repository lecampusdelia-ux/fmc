...
import type { ActionAlert, FmcAction, DeployAlert, FileHistory, SupabaseAction, SupabaseAlert } from '~/types/actions';
import { createScopedLogger } from '~/utils/logger';
import { unreachable } from '~/utils/unreachable';
import type { ActionCallbackData } from './message-parser';
import type { FmcShell } from '~/utils/shell';

const logger = createScopedLogger('ActionRunner');

export type ActionStatus = 'pending' | 'running' | 'complete' | 'aborted' | 'failed';

export type BaseActionState = FmcAction & {
...
export type FailedActionState = FmcAction &
...
export class ActionRunner {
  ...
  #shellTerminal: () => FmcShell;
  #persistentProcesses = new Map<string, WebContainerProcess>();
  ...
  addAction(data: ActionCallbackData) {
    ...
    this.actions.setKey(actionId, {
      ...
      abort: () => {
        ...
        const persistentProcess = this.#persistentProcesses.get(actionId);

        if (persistentProcess) {
          persistentProcess.kill();
          this.#persistentProcesses.delete(actionId);
        }
    ...
    try {
      switch (action.type) {
        ...
        case 'start': {
          ...
          console.log(`[ActionRunner] Starting persistent process for ${actionId}: ${action.content}`);
  ...
  async #runShellAction(action: ActionState) {
    ...
    if (resp?.exitCode != 0 && resp?.exitCode !== 130) {
      if (action.abortSignal.aborted) {
        return;
      }
  ...
  async #runStartAction(action: ActionState, actionId: string) {
    console.log(`[ActionRunner] #runStartAction: ${actionId}`);
    ...
    this.#persistentProcesses.set(actionId, process);
