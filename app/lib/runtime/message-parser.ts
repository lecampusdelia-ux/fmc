import type { ActionType, FmcAction, FmcActionData, FileAction, ShellAction, SupabaseAction } from '~/types/actions';
import type { FmcArtifactData } from '~/types/artifact';
import { createScopedLogger } from '~/utils/logger';
import { unreachable } from '~/utils/unreachable';

const ARTIFACT_TAG_OPEN = '<fmcArtifact';
const ARTIFACT_TAG_CLOSE = '</fmcArtifact>';
const ARTIFACT_ACTION_TAG_OPEN = '<fmcAction';
const ARTIFACT_ACTION_TAG_CLOSE = '</fmcAction>';
const FMC_QUICK_ACTIONS_OPEN = '<fmc-quick-actions>';
const FMC_QUICK_ACTIONS_CLOSE = '</fmc-quick-actions>';

const logger = createScopedLogger('MessageParser');

export interface ArtifactCallbackData extends FmcArtifactData {
  messageId: string;
  artifactId?: string;
}
...
interface MessageState {
  ...
  currentArtifact?: FmcArtifactData;
  currentAction: FmcActionData;
  actionId: number;
}
...
export class StreamingMessageParser {
  ...
  parse(messageId: string, input: string) {
    ...
    while (i < input.length) {
      if (input.startsWith(FMC_QUICK_ACTIONS_OPEN, i)) {
        const actionsBlockEnd = input.indexOf(FMC_QUICK_ACTIONS_CLOSE, i);

        if (actionsBlockEnd !== -1) {
          const actionsBlockContent = input.slice(i + FMC_QUICK_ACTIONS_OPEN.length, actionsBlockEnd);

          // Find all <fmc-quick-action ...>label</fmc-quick-action> inside
          const quickActionRegex = /<fmc-quick-action([^>]*)>([\s\S]*?)<\/fmc-quick-action>/g;
          let match;
          const buttons = [];
          ...
          i = actionsBlockEnd + FMC_QUICK_ACTIONS_CLOSE.length;
          continue;
        }
      }
        ...
        if (state.insideAction) {
          ...
          if (closeIndex !== -1) {
            ...
            this._options.callbacks?.onActionClose?.({
              ...
              action: currentAction as FmcAction,
            });
          ...
          if (actionOpenIndex !== -1 && (artifactCloseIndex === -1 || actionOpenIndex < artifactCloseIndex)) {
            ...
            if (actionEndIndex !== -1) {
              ...
              this._options.callbacks?.onActionOpen?.({
                ...
                action: state.currentAction as FmcAction,
              });
...
function createQuickActionElement(props: Record<string, string>, label: string) {
  const elementProps = [
    'class="__fmcQuickAction__"',
    'data-fmc-quick-action="true"',
...
function createQuickActionGroup(buttons: string[]) {
  return `<div class="__fmcQuickActionGroup__" data-fmc-quick-action="true">${buttons.join('')}</div>`;
}
