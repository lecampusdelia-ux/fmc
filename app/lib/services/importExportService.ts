...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          fmc_user_profile: this._safeGetItem('fmc_user_profile'),
          fmc_settings: this._safeGetItem('fmc_settings'),
          fmc_profile: this._safeGetItem('fmc_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('fmc_viewed_features'),
          developer_mode: this._safeGetItem('fmc_developer_mode'),
        ...
        ui: {
          // Tab configuration
          fmc_tab_configuration: this._safeGetItem('fmc_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('fmc_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('fmc_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          fmc_read_logs: this._safeGetItem('fmc_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('fmc_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('fmc_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.fmc_tab_configuration) {
        try {
          this._safeSetItem('fmc_tab_configuration', data.ui.fmc_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'fmc_acknowledged_debug_issues',
        'fmc_acknowledged_connection_issue',
        'error_logs',
        'fmc_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('fmc_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
