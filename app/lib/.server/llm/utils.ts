...
export function simplifyfmcActions(input: string): string {
  // Using regex to match fmcAction tags that have type="file"
  const regex = /(<fmcAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/fmcAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<fmcAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</fmcAction>`;
    });

  return `<fmcArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</fmcArtifact>`;
}
